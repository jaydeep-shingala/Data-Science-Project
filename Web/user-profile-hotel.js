...
firebase.auth().onAuthStateChanged(function(user) {
  ...
  if (user) {
	...
	//window.alert(userId);
	var userRef = firebase.database().ref().child('/hotel/' + 'users');
	...
	var ref2 = firebase.database().ref().child('/hotel/' + userId);
	var c=0;
...
auth.sendPasswordResetEmail(email).then(function() {
  ...
  var span = document.getElementsByClassName("close")[0];
   var c="An mail has been sent to your registred email address!";
...
window.onclick = function(event) {
  ...
  // An error happened.
  //window.alert(error);
  document.getElementById("mas").innerHTML = error;
});
}
function myf(){
	//window.alert("hi");
